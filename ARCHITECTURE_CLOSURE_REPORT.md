...
| Projection | `projection.py` | `project` | predicate / compatibility wrapper | Clean structural concern. Its deliberately narrow `ProjectedIR` carries only removal and traversal inputs; projection does not classify, serialize, or hash. |
...
| `ProjectedIR` | `projection.py` | predicate, projection wrapper | `serialization.projected_ir_to_dict` | `serialization.projected_ir_identity_hash` | Narrow traversal object; legacy `complement_projection` wrapper preserves its three-field dictionary |
