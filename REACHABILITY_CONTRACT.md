...
9. Deterministic ordering is lexical by normalized identifiers unless an existing normalized IR array already defines a canonical lexical order; visited nodes, reached roots, traversal edges, and diagnostics must be emitted in deterministic lexical order.
10. The reachability result hash boundary includes the canonical JSON result payload excluding `reachability_result_hash` itself and excluding no other result fields.
...
Diagnostics are deterministic structural messages scoped to reachability only. The canonical unreachable diagnostic code is `REACHABILITY.UNREACHABLE_TARGET`.

## Determinism and hash boundary

Reachability result documents must be serialized for hash purposes as canonical UTF-8 JSON bytes with sorted object keys and compact separators. If `reachability_result_hash` is present, it is the SHA-256 hash of the result document after removing only `reachability_result_hash`.
